...
def test_compose_specification_factory():
    compose_strategy = ComposeSpecificationFactory()
    assert(isinstance(compose_strategy([]), ComposeSpecification))
...
services:
  ...
  web:
    ...
    depends_on:
      - db
    """
    assert(isinstance(compose_strategy([texts]), ComposeSpecification))
